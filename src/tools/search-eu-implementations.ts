...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_russian_implementation?: boolean;
  limit?: number;
}
...
/**
 * Search for EU directives/regulations with Russian implementation information.
 */
export async function searchEUImplementations(
  ...
  try {
    ...
    // Filter by Russian implementation existence
    if (input.has_russian_implementation !== undefined) {
      if (input.has_russian_implementation) {
        sql += ` HAVING russian_statute_count > 0`;
      } else {
        sql += ` HAVING russian_statute_count = 0`;
      }
    }
    ...
    const results = rows.map(row => ({
      ...
      russian_statute_count: row.russian_statute_count,
    ...
    if (msg.includes('no such table')) {
      return {
        ...
        _metadata: generateResponseMetadata(db),
      } as unknown as ToolResponse<SearchEUImplementationsResult>;
    }
    throw err;
  }
}
